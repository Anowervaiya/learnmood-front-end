...
const [submitting, setSubmitting] = useState(false);
  const previewUrl = files[0]?.preview || fetchedImageUrl || null;
    ...
    <div className="flex flex-col items-center gap-2">
      <div className="relative inline-flex">
        ...
        <button
          ...
          disabled={disabled}
                 className="relative flex size-28 items-center justify-center overflow-hidden rounded-full border border-input 
                  outline-none transition-colors bg-accent
                   focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50 has-disabled:pointer-events-none has-[img]:border-none has-disabled:opacity-50 data-[dragging=true]:bg-accent/50"
        ...
        >
          {finalImageUrl ? (
            <img
              alt="User avatar"
              className="size-full h-full w-full    rounded-full object-cover"
      ...
      <Dialog
        onOpenChange={(val) => !disabled && setIsDialogOpen(val)}
          ...
          <DialogHeader className="contents space-y-0 text-left">
            <DialogTitle className="flex items-center justify-between border-b p-4 text-base">
              ...
              <Button
                autoFocus
                className="-my-1"
                disabled={!previewUrl || disabled || submitting}
                onClick={handleApply}
              >
